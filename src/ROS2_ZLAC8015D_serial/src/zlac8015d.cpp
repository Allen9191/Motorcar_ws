...
float ZLAC::get_rpm()
{
    ...
    float rpm = static_cast<float>(rpm_tenth)/10.0f;
    printf("Get RPM =  %.1f\n", rpm);
    return (float)rpm;
}
...
uint8_t ZLAC::read_hex(uint8_t num_bytes)
{
    ...
    if (line.size() >= 6) {
        uint8_t data_hi = receive_hex[4];
        uint8_t data_lo = receive_hex[5];
        uint16_t u16    = (static_cast<uint16_t>(data_hi) << 8) | data_lo;
        int16_t  s16    = static_cast<int16_t>(u16);  // 若該暫存器是有號值可參考

        printf("0x%02X 0x%02X | rpm=%d\n", data_hi, data_lo, s16);
    } else {
